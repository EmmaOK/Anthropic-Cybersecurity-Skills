...
SPECIALIST_PERSONAS: dict[str, str] = {
    ...
    "ai": (
        ...
        "LLM01→detecting-ai-model-prompt-injection-attacks (--scan-file <log.jsonl> or --text <input>), "
        "LLM02→llm-output-validation-and-sanitization (--model <model> --system-prompt <prompt>), "
        "LLM03→llm-data-and-model-poisoning-defense (dataset-scan --dataset <file> OR backdoor-probe --model <model>), "
        "LLM06→llm-sensitive-information-disclosure-prevention (--model <model> --system-prompt <prompt>), "
        "LLM07→llm-system-prompt-leakage-prevention (--model <model> --system-prompt <prompt>), "
        "LLM08_agency→llm-excessive-agency-prevention (--manifest tools.json --function <desc>), "
        "LLM08_vectors→llm-vector-and-embedding-security (--vector-db-url <url> OR --scan-dir <dir>), "
        "ASI01→agent-goal-hijacking-detection (--goal <goal> --log agent.jsonl), "
        "ASI02→agentic-tool-misuse-detection-and-control (--manifest tools.json), "
        "ASI03→agent-identity-and-privilege-governance (--config agent_config.json), "
        "ASI04→agentic-supply-chain-integrity-verification (--requirements requirements.txt), "
        "ASI05→agent-code-execution-sandboxing (--config sandbox_config.json), "
        "ASI06→agent-memory-poisoning-detection-and-defense (--log memory_log.jsonl), "
        "ASI07→securing-inter-agent-communication-channels (--agent-url <url>), "
        "ASI08→agentic-cascading-failure-prevention (--config system_config.json), "
        "ASI09→human-agent-trust-and-oversight-controls (--log interactions.jsonl), "
        "ASI10→rogue-agent-detection-and-containment (--log telemetry.jsonl), "
        "MCP01→mcp-tool-poisoning-detection-and-defense (--server-url <url>), "
        "MCP02→mcp-authentication-and-authorization-hardening (--server-url <url>), "
        "MCP03→mcp-privilege-scope-enforcement (--server-url <url>), "
        "MCP04→mcp-context-injection-prevention (--server-url <url>), "
        "MCP05→mcp-shadow-server-discovery (--server-url <url>), "
        "MCP06→mcp-command-injection-prevention (--server-url <url>), "
        "MCP07→mcp-token-management-and-secret-protection (--server-url <url> [--scan-dir <dir>]), "
        "MCP08→mcp-intent-flow-subversion-detection (--server-url <url> [--log tool_calls.jsonl]), "
        "MCP09→mcp-audit-logging-and-telemetry-setup (--server-url <url> [--log-file audit.jsonl]), "
        "MCP10→mcp-supply-chain-security-assessment (--server-url <url> [--requirements req.txt] [--scan-env]). "
